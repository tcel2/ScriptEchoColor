...
while true;do
  ...
  for strFolder in "${CFGastrFolderList[@]}";do
    ...
    #strStatusNew="`ls -lR --time-style=full-iso "$strFolder/"* "$strFolder/".*`"&&:  #TODO needs to list the hidden files too recusrively
    strStatusNew="`find "$strFolder/" -exec stat -c "\"%n\" \"%y\" %Y" '{}' \;`"&&:
