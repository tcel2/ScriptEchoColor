...
#          <Key mask="S4" key="M">exec:'"xterm -e \"$0 --script cicleGammaBack\" #kill=skip"'</Key>
    ...
    # DOCUMENTATION FROM JWM:
		...
		#	5 - mod5
    echo -e '
	        <Key key="XF86AudioRaiseVolume">exec:bash -c "FUNCvolume --up"</Key>
	        <Key key="XF86AudioLowerVolume">exec:bash -c "FUNCvolume --down"</Key>
	        <Key key="XF86AudioMute">exec:bash -c "FUNCvolume --togglemute"</Key>
